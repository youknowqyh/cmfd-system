...
class SelfCorrelationPercPooling(Layer):
    ...
    def call(self, x):
        ...
        nb_maps = nb_rows * nb_cols # 256
        # self correlation
        # bsize, 256, 512
        x_3d = K.reshape(x, (-1, nb_maps, nb_feats))
        # bsize, 256, 256
        x_corr_3d = tf.matmul( x_3d, x_3d, transpose_a = False, transpose_b = True ) / nb_feats
        # bsize, 16, 16, 256
        x_corr = K.reshape( x_corr_3d, (-1, nb_rows, nb_cols, nb_maps) )
...
def create_cmfd_similarity_branch(img_shape=(256,256,3), \
                    nb_pools=100, name='simiDet'):
    # ---------------------------------------------------
    # Input
    # ---------------------------------------------------
    img_input = Input(shape=img_shape, name=name+'_in')
    # ---------------------------------------------------
    # VGG16 Conv Feature extractor
    # ---------------------------------------------------    
    bname = name + '_cnn'
    ## Block 1
...
def create_cmfd_testing_model(weight_file=None):
    '''create a cmfd testing model with pretrained weights
    '''
    ...
    ManiDet = Model( inputs=mani_branch.inputs,
                     ...
                     name='maniFeatex' )
    # 3. define the two-branch CMFD model
    ...
    # 4. create CMFD model end-to-end
    model = Model( inputs = img_raw, outputs = mask_out, name = 'CMFDNet')
...
def create_simi_only_model(weight_file=None):
    cmfdmodel = create_cmfd_testing_model(weight_file)

    model = create_cmfd_similarity_branch()
    img_raw = Input( shape=(None,None,3), name='image_in')
    img_in = Preprocess( name='preprocess')( img_raw )   
    mask_out = model(img_in) 
    simimodel = Model( inputs = img_raw, outputs = mask_out, name = 'CMFDNet')
    for layer in cmfdmodel.get_layer('simiFeatex').layers:
        simimodel.get_layer('simiDet').get_layer(layer.name).set_weights(layer.get_weights())
    return simimodel